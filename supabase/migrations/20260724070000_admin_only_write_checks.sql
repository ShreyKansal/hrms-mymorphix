-- Module 21 PRD's own explicit framing principle: "frontend restrictions are UX, not security —
-- all permissions must be enforced on the backend." The UI now hides Add Employee/Transfer
-- buttons from non-admins, but create_employee()/transfer_employee() themselves never checked
-- the caller's role — a non-admin could still call supabase.rpc('transfer_employee', {...})
-- directly and it would succeed, tenant-ownership checks notwithstanding. That's exactly the
-- gap this module exists to close; leaving it open while building Module 21 would contradict
-- the principle stated in this same migration set's own comments.
...
begin
  ...
  select role into v_caller_role from public.profiles where id = auth.uid();
  if v_caller_role is distinct from 'admin' then
    raise exception 'Only admins can transfer employees';
  end if;
